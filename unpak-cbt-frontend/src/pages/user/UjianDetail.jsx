...
const UjianMabaDetail = () => {
        ...
        const validateExamStatus = async () => {
            ...
            try {
                ...
                if (statusResponse.data.status == "active") {
                    alert(
                        "Ujian tidak dapat diakses karena status belum mulai ujian.",
                    );
                    navigate(`/maba/${examData.idUjian}/${examData.noReg}`);
                }
                if (statusResponse.data.status == "done") {
                    alert(
                        "Ujian tidak dapat diakses karena status sudah selesai.",
                    );
                    navigate(`/maba/${examData.idUjian}/${examData.noReg}`);
                }
            } catch (error) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const countedAnswers = jawabanResponse.data.reduce(
                    (acc, item) => {
                        if (
                            item.uuidTemplatePilihan &&
                            item.trial == (isTrial ? "True" : "False")
                        ) {
        ...
        if (jadwalUjian) {
            timer = setInterval(() => {
                ...
                const start = new Date(
                    `${jadwalUjian.tanggal}T${jadwalUjian.jamMulai}:00+07:00`,
                );
                const end = new Date(
                    `${jadwalUjian.tanggal}T${jadwalUjian.jamAkhir}:00+07:00`,
                );

                if (nowWib < start) {
                    setStatus("waiting");
                    setTimeLeft(Math.floor((start - nowWib) / 1000));
                } else if (nowWib >= start && nowWib <= end) {
                    setStatus("ongoing");
                    setTimeLeft(Math.floor((end - nowWib) / 1000));
                } else {
                    setStatus("finished");
                    setTimeLeft(0);
                }
    ...
    const handleSelesaiUjian = async () => {
        ...
        if (
            confirm(
                "Anda yakin ingin selesai ujian? pastikan kembali jawaban sudah terjawab semua.",
            ) == true
        ) {
            try {
                const payload = {
    ...
    return (
        <>
            <Suspense
                fallback={<LoadingScreen message="Sedang memuat data..." />}
            >
                <NavbarMaba>
                ...
                <div className="container mx-auto p-4">
                            ...
                            .map((tipe) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {tipe === "MTK" && (
                                            <FaSquareRootAlt className="text-4xl" />
                                        )}
                                        {tipe === "BI" && (
                                    ...
                                    <span className="mt-2 text-sm text-gray-600">
                                        {answeredCount[tipe] || 0} /{" "}
                                        {groupedPertanyaan[tipe].length} soal
                                        terjawab
                                    </span>
                                </div>
                            ))}
                    ...
                    {status === "finished" && (
                        <div className="mt-8 flex items-center justify-center text-red-500 font-semibold bg-red-100 text-red-700 p-4 rounded-lg">
                            <FaTimesCircle className="text-2xl mr-2" /> Ujian
                            telah berakhir.
                        </div>
                    )}
